import { meses, classificacoes } from "@/data/obzData";
...
export default function DashboardFilters({ mesFilter, setMesFilter, classificacaoFilter, setClassificacaoFilter }: Props) {
  return (
    <div className="flex flex-wrap gap-3">
      <Select
        value={mesFilter !== null ? String(mesFilter) : "all"}
        onValueChange={(v) => setMesFilter(v === "all" ? null : Number(v))}
      >
      ...
      <Select
        value={classificacaoFilter || "all"}
        onValueChange={(v) => setClassificacaoFilter(v === "all" ? null : v)}
      >
